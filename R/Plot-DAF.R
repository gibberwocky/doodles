...
fname <- "S2_JFM-OTH.daf"
dat <- read.table(fname, header=F, stringsAsFactors=F)
names(dat) <- c("chr", "start", "JFM", "OTH", "DAF", "zDAF")


# Pick a value to plot
names(dat)[6] <- "Val"
plot_x_label <- "DAF"
chromosome <- 6 # Set to 0 plots chromosomes 1-16
save2pdf <- 0
