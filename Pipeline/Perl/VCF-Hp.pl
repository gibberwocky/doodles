...
print $fh_counts "chr \t pos \t n_ref \t n_alt \n";

# Open a file to output bin results to
my $file_bins ="$ARGV[0].hp";
open(my $fh_bins, '>', $file_bins) or die "Could not open file '$file_bins' $!";
print $fh_bins "bin_chr \t bin_start \t bin_stop \t chr \t prem_snp \t dern_snp \t n_snps \t sum_Maj \t sum_Min \t Hp \n";
	...
	if (substr($data, 0, 1) eq "#") {
		...
		if ( $vcf[0] == $bin_chr && $vcf[1] >= $bin_start && $vcf[1] <= $bin_stop )
		{
			...
			do {
				...
				# Write vales to file
	                        print $fh_bins "$bin_chr \t $bin_start \t $bin_stop \t $tmp_chr \t $tmp_start \t $tmp_stop \t $tmp_snps \t $sum_Maj \t $sum_Min \t $Hp \n";
