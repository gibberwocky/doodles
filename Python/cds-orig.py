...
try:
        ...
        curs.execute('CREATE TABLE output (id INTEGER PRIMARY KEY AUTOINCREMENT, gene TEX, chr TEXT, start INT, end INT, cds INT, missense INT, synonymous INT, freqmis REAL, freqsyn REAL, alpha REAL, pval REAL, pfdr REAL, snpmis TEXT, snpsyn TEXT) ')
        conn.commit()
...
for chr in chr_list:
	...
	for row in set(gene_unique):
		...
		# Calcualte alpha
		# (tmp_gene, tmp_chr, tmp_start, tmp_end, tmp_cds, tmp_missense, tmp_synonymous, tmp_freqmis, tmp_freqsyn, tmp_d, tmp_snpmis, tmp_snpsyn))
		# alpha = dat[,8]^2 * (dat[,6]*N + dat[,7]*P) / (dat[,5] * as.numeric((N+P)))
		alpha = 0
		alpha = float(tmp_freqmis)**2 * ( int(tmp_missense) * int(vep_mis) + int(tmp_synonymous) * int(vep_syn) ) / ( int(tmp_cds) * ( int(vep_mis) + int(vep_syn) ))

		# Fit exponential distribution and pull p-values
		# Currently performed in R
#		param = expon.fit(alpha)
		pval = 0		
		pfdr = 0

		# Write to table
		curs.execute('INSERT INTO output (gene, chr, start, end, cds, missense, synonymous, freqmis, freqsyn, alpha, pval, pfdr, snpmis, snpsyn) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)', (tmp_gene, tmp_chr, tmp_start, tmp_end, tmp_cds, tmp_missense, tmp_synonymous, tmp_freqmis, tmp_freqsyn, alpha, pval, pfdr, tmp_snpmis, tmp_snpsyn))
		conn.commit()


# Output to file
printout("\nWriting OUTPUT to : %s\n\n" % (outfile+'.csv'), RED)
curs.execute('SELECT gene, chr, start, end, cds, missense, synonymous, freqmis, freqsyn, alpha, pval, pfdr, snpmis, snpsyn FROM output')
tmp = curs.fetchall()
with open((outfile+'.csv'), 'wb') as fh:
	writer = csv.writer(fh, delimiter='\t')
	writer.writerow(['GENE', 'CHR', 'START', 'END', 'CDS', 'N_MISSENSE', 'N_SYNON', 'FQ_MISSENSE', 'FQ_SYNON', 'ALPHA', 'PVAL', 'PFDR', 'SNPs_MISSENSE', 'SNPs_SYNON'])
