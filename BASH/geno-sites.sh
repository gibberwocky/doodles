...
usage()
{
  ...
  echo -e "\n\e[96mDetails\e[0m"
  echo -e "\e[92mGenotypes list of samples for a given set of sites.\e[0m"
  echo -e "\n\e[96mOPTIONS:\e[0m"
...
while getopts ":i:o:b:n:s:p:" opt; do
  case $opt in
    i) INFILE=${OPTARG};;
    o) DUMP=${OPTARG};;
    b) BAMS=${OPTARG};;
...
if [[ -z ${INFILE} ]] | [[ -z ${DUMP} ]] | [[ -z ${BAMS} ]] | [[ -z ${NAME} ]] | [[ -z ${SITES} ]]
then
  usage
  exit 1
fi
...
# Clean up
rm ${DUMP}/${NAME}_subhet.vcf.gz
rm ${DUMP}/${NAME}_subhet.vcf.gz.tbi
